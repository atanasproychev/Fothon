...
class ForumViewsTestCase(TestCase):
    def setUp(self):
        ...
        category = Category.objects.create(title='Second', order=2,
                                           parent=main)
        Category.objects.create(title='ThirdSpecial', order=3,
                                parent=main, type='S')
        subcategory = Category.objects.create(title='SubCategory',
                                              order=1, parent=category)
        Category.objects.create(title='SubCategorySpecial', order=2,
                                parent=category, type='S')
        user = ForumUser.objects.create_user(username='specialuser',
                                             password='222222')
        group = Group.objects.create(name='SpecialUser')
        user.groups = [group]
        Topic.objects.create(title='Topic', author=user,
                             category=subcategory, last_modified_from=user)
    ...
    def test_new_content_view_create_post(self):
        ...
        response = self.client.post("/fothon/post/1/new/?back=/fothon/",
                                    {'content': "New post"})
    ...
    def test_new_content_view_create_topic(self):
        ...
        response = self.client.post("/fothon/topic/2/new/?back=/fothon/",
                                    {'content': "New topic"})
    ...
    def test_login_invalid_username(self):
        response = self.client.post("/fothon/login/", {
            'username': "username", 'password': "password"})
        self.assertEqual(response.status_code, 200)

    def test_login(self):
        response = self.client.post("/fothon/login/", {
            'username': "specialuser", 'password': "222222"})
        self.assertEqual(response.status_code, 302)

    def test_register(self):
        response = self.client.post("/fothon/register/", {
            'username': "username", 'password': "password",
    ...
    def test_register_already_existing_user(self):
        response = self.client.post("/fothon/register/", {
            'username': "specialuser", 'password': "password",
            'email': "email@example.com"})
        self.assertEqual(response.status_code, 200)
    ...
    def test_search_post(self):
        ...
        response = self.client.post("/fothon/search/", {
            'search_field': "text", 'type': "post",
            'author_username': "", 'date_field': ""})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "text")

    def test_search_topic(self):
        response = self.client.post("/fothon/search/", {
            'search_field': "Topic", 'type': "topic",
            'author_username': "", 'date_field': ""})
        self.assertEqual(response.status_code, 200)
    ...
    def test_search_category(self):
        response = self.client.post("/fothon/search/", {
            'search_field': "Second", 'type': "category",
            'author_username': "", 'date_field': ""})
        self.assertEqual(response.status_code, 200)
